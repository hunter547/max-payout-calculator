...
function verdict(
  ...
  /** Set where the firm ends the account after a count of payouts. */
  spent: { firm: string; maxPayouts: number } | null,
) {
  // Nothing to plan for: the firm closes the account after its last payout,
  // and the next one comes with a new account.
  if (spent) {
    return {
      title: "This account is finished",
      detail: `${spent.firm} closes an account after ${spent.maxPayouts} payouts. Qualifying again starts a new one, and its schedule begins at the first payout.`,
    };
  }
  if (consistencyInvalid) {
    return {
      title: "Set a consistency rule above 0%",
      detail: "The daily cap and the number of trading days both depend on it.",
    };
  }
  ...
  if (results.targetMet) {
    const bar =
      qualifyingDayProfit > 0 ? formatCurrency(qualifyingDayProfit) : null;
    const dayWords =
      days === 1
        ? "One more trading day"
        : `${countWord(days)} more trading days`;
    return {
      title: bar
...
export default function App() {
    ...
    const initial: SetupDraft =
      setup && !adding
        ? {
            ...
            payoutTaken:
              setup.approach === "dayByDay" ? setup.payoutTaken : null,
            strategy,
            curated,
            balance: account.balance,
            largestProfitDay: snapshot.largestProfitDay,
  ...
  // Past the firm's last payout there is nothing to plan for, so the plan
  // itself comes off the page along with the target it was aiming at.
  const spent = payoutsSpent(schedule, payoutsSoFar);
  ...
  const { title, detail } = verdict(
    ...
    inclusiveBar,
    spent
      ? { firm: firmOf(template).name, maxPayouts: schedule.maxPayouts ?? 0 }
      : null,
  );
  ...
  function startingCurated(): CuratedDraft {
    if (fastest <= MAX_SPELLED_COUNT) {
      ...
      return {
        ...curated,
        mode: "days",
        days: Math.min(start, MAX_SPELLED_COUNT),
      };
    }
    return {
      ...curated,
      mode: "cap",
      cap: results.maxAllowedSingleDay.toFixed(2),
    };
  }
                ...
                payout={
                  hasSchedule(template)
                    ? {
                        ...
                        repeats:
                          nextPayoutNumber(payoutsSoFar) >=
                          schedule.caps.length,
                      }
                    : null
                }
                tradingDays={
                  approach === "dayByDay"
                    ? {
                        logged: summary.tradingDays,
                        counting: summary.qualifyingDays,
                      }
                    : null
                }
                consistencyInvalid={consistencyInvalid}
